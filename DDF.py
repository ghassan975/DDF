    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                ...
                print("[✘] Inv𝐃𝐄𝐊𝐎d cookie [NO]");time.sleep(2);self.login_cokie()
...
def passwrd():
	...
	print('')
	print('>> Lanjut Crack Kemb𝐃𝐄𝐊𝐎 ( Y/t ) ? ')
	woi = input('>> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
@SSWSG   -  @SSWSG 					
					...
					try:ttl = re.findall('\<\/td\>\<td\ v𝐃𝐄𝐊𝐎gn\=\"top\" class\=\".*?\"\>\<div\ class\=\".*?\"\>(\d+\s+\w+\s+\d+)<\/div\>\<\/td\>\<\/tr\>',str(response))[0]
					except:ttl=""
